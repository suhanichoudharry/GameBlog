...
function analyzeClassNameExpression(node) {
	...
	if (t.isJSXExpressionContainer(node)) return analyzeClassNameExpression(node.expression);
	if (t.isTemplateLiteral(node) && node.expressions.length === 0) return {
		type: "static",
		value: node.quasis.map((q) => q.value.raw).join("")
	};
	return { type: "dynamic" };
}
function analyzeElementDeletionSafety(path$1) {
	let current = path$1.parent;
	while (current) {
		if (t.isConditionalExpression(current)) return {
			type: "dynamic",
			reason: "conditional-expression"
		};
		if (t.isCallExpression(current) && t.isMemberExpression(current.callee) && t.isIdentifier(current.callee.property) && current.callee.property.name === "map") return {
			type: "dynamic",
			reason: "map-expression"
		};
		if (t.isLogicalExpression(current) || t.isSequenceExpression(current) || t.isArrayExpression(current)) return {
			type: "dynamic",
			reason: "complex-parent"
		};
		if (t.isFunction(current) || t.isArrowFunctionExpression(current) || t.isJSXElement(current)) break;
...
function extractTextContent(children) {
	...
	if (children.some((child) => t.isJSXElement(child) || t.isJSXFragment(child))) return {
		type: "dynamic",
		reason: "has-children"
	};
	if (children.every((child) => t.isJSXText(child) || t.isJSXExpressionContainer(child) && t.isStringLiteral(child.expression))) return {
		type: "static",
		value: children.map((child) => {
			if (t.isJSXText(child)) return child.value;
			else if (t.isJSXExpressionContainer(child) && t.isStringLiteral(child.expression)) return child.expression.value;
			return "";
		}).join("").trim()
	};
	return {
...
async function analyzeElement(componentId, rootDir) {
	try {
		...
		traverse$1(ast, { JSXElement(path$1) {
			const openingElement = path$1.node.openingElement;
			const loc = openingElement.loc;
			if (!loc) return;
			if (loc.start.line === line && Math.abs(loc.start.column - col) <= 5) {
				elementFound = true;
				const classNameAttr = openingElement.attributes.find((attr) => t.isJSXAttribute(attr) && t.isJSXIdentifier(attr.name) && attr.name.name === "className");
				if (classNameAttr) classNameAnalysis = analyzeClassNameExpression(classNameAttr.value);
				else classNameAnalysis = {
					type: "static",
					value: ""
				};
				const children = path$1.node.children;
				textContentAnalysis = extractTextContent(children);
				elementTypeAnalysis = analyzeElementDeletionSafety(path$1);
				path$1.stop();
			}
		} });
		if (elementFound) {
...
async function updateComponentElement(componentId, updates, rootDir) {
	try {
		...
		traverse(ast, { JSXElement(path$1) {
			const openingElement = path$1.node.openingElement;
			const loc = openingElement.loc;
			if (!loc) return;
			foundElements++;
			...
			if (loc.start.line === line && Math.abs(loc.start.column - col) <= 5) {
				if (updates.className !== void 0) {
					...
					else if (classNameAttr) classNameAttr.value = t.stringLiteral(updates.className);
					else openingElement.attributes.push(t.jsxAttribute(t.jsxIdentifier("className"), t.stringLiteral(updates.className)));
				}
				if (updates.textContent !== void 0) {
					path$1.node.children = [];
					if (updates.textContent.trim() !== "") path$1.node.children.push(t.jsxText(updates.textContent));
				}
				modified = true;
				path$1.stop();
			}
		...
		resolveId: {
			...
			handler: async (source, importer, _options) => {
				...
				if (importer) {
					...
					} else resolvedPath = path.resolve(path.dirname(importer), source);
...
//#endregion
export { src_default as default, dynamicComponentCreatorPlugin, hercules };
